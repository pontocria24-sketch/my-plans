  ...
  return (
    <div className="h-[calc(100vh-100px)] lg:h-[calc(100vh-140px)] flex flex-col bg-slate-900 border border-slate-800 rounded-2xl lg:rounded-[3rem] overflow-hidden shadow-2xl animate-in fade-in duration-500">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* MAIN GRID */}
        <main className="flex-1 bg-slate-950/20 overflow-y-auto custom-scrollbar">
           {viewMode === 'Month' && (
             <div className="grid grid-cols-7 h-full border-l border-slate-800">
                {weekDays.map(d => (
                  <div key={d} className="bg-slate-900 border-b border-r border-slate-800 py-4 text-center">
                    <span className="text-[10px] font-black text-slate-600 uppercase tracking-[0.4em]">{d}</span>
                  </div>
                ))}
                {Array.from({ length: firstDayOfMonth(currentDate) }).map((_, i) => <div key={`empty-main-${i}`} className="border-b border-r border-slate-800/20 bg-slate-950/10" />)}
                {Array.from({ length: daysInMonth(currentDate) }).map((_, i) => {
                  const day = i + 1;
                  ...
                  return (
                    <div key={day} className={`min-h-[160px] border-b border-r border-slate-800/40 p-4 transition-all group hover:bg-slate-900/40 ${isToday ? 'bg-indigo-600/5' : ''}`}>
                      <div className="flex justify-between items-center mb-4">
                         <span className={`text-sm font-black ${isToday ? 'text-indigo-400' : 'text-slate-700'}`}>{day}</span>
                      </div>
           ...
           {viewMode === 'Week' && (
              <div className="grid grid-cols-7 h-full divide-x divide-slate-800 border-l border-slate-800">
                 {weekEvents.map((dayData, i) => {
                    ...
                    return (
                       <div key={i} className={`flex flex-col min-h-full ${isToday ? 'bg-indigo-600/5' : ''}`}>
                          <div className="p-6 border-b border-slate-800 bg-slate-900 text-center">
                             <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">{weekDays[dayData.date.getDay()]}</p>
                             <p className={`text-3xl font-black ${isToday ? 'text-indigo-400' : 'text-slate-700'}`}>{dayData.date.getDate()}</p>
                          </div>
                          <div className="p-4 space-y-3 flex-1 overflow-y-auto custom-scrollbar">
                             {dayData.items.map(item => (
                                <div key={item.id} onClick={() => setSelectedEvent(item)} className={`p-4 rounded-3xl border cursor-pointer hover:border-indigo-500/30 transition-all ${getEventColor(item)}`}>
           ...
           {viewMode === 'Day' && (
              <div className="p-12 space-y-8 animate-in slide-in-from-right-4 max-w-4xl mx-auto">
                 <div className="mb-12">
                    <h3 className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.4em] mb-2">CRONOGRAMA DO DIA</h3>
                    <h2 className="text-5xl font-black text-white tracking-tighter uppercase">{selectedDay.toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' })}</h2>
                 </div>

                 <div className="space-y-6 relative pl-12 border-l border-slate-800 ml-6">
                    {Array.from({ length: 24 }).map((_, hour) => {
                       ...
                       return (
                          <div key={hour} className="relative py-2 min-h-[80px]">
                             <div className="absolute -left-[58px] top-4 text-[10px] font-black text-slate-700 tracking-widest tabular-nums w-10 text-right">{hourStr}:00</div>
                             <div className="absolute -left-[14px] top-5 w-3 h-3 rounded-full bg-slate-800 border-4 border-slate-900 shadow-xl"></div>
                             
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {hourItems.map(item => (
                                   <div key={item.id} onClick={() => setSelectedEvent(item)} className={`p-6 rounded-[2.5rem] shadow-xl border border-slate-800/40 hover:scale-[1.02] transition-all cursor-pointer ${getEventColor(item)}`}>
           ...
           {viewMode === 'Year' && (
              <div className="p-12 animate-in fade-in duration-700">
                 <div className="mb-12">
                    <h3 className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.4em] mb-2">VISUALIZAÇÃO ANUAL</h3>
                    <h2 className="text-5xl font-black text-white tracking-tighter uppercase">{currentDate.getFullYear()}</h2>
                 </div>
      ...
      {selectedEvent && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 backdrop-blur-2xl animate-in fade-in duration-300">
           ...
           <div className="bg-slate-900 border border-slate-800 w-full max-w-md rounded-[3.5rem] overflow-hidden relative shadow-2xl animate-in zoom-in-95 duration-300">
              ...
              <div className="p-10 border-b border-slate-800 bg-slate-900/50 flex justify-between items-center">
                 <h3 className="text-2xl font-black text-white tracking-tighter uppercase">{selectedEvent.title}</h3>
                 <button onClick={() => setSelectedEvent(null)} className="p-3 bg-slate-800 hover:bg-slate-700 rounded-full text-slate-400 transition-all"><X className="w-5 h-5" /></button>
              </div>
              <div className="p-10 space-y-8">
                 <div className="flex items-center gap-5">
                    <div className="p-3 bg-indigo-500/10 rounded-2xl border border-indigo-500/20"><Clock className="w-6 h-6 text-indigo-400" /></div>
                    <div>
                       <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest mb-1">HORÁRIO</p>
                       <p className="text-base font-black text-white uppercase">{new Date(selectedEvent.date + 'T00:00:00').toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: 'long' })} • {selectedEvent.time}</p>
                    </div>
                 </div>

                 {selectedEvent.link && (
                   <div className="flex items-center gap-5">
                      <div className="p-3 bg-emerald-500/10 rounded-2xl border border-emerald-500/20"><LinkIcon className="w-6 h-6 text-emerald-400" /></div>
                      <div className="flex-1 overflow-hidden">
                         <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest mb-1">LINK DE ACESSO</p>
                         <a href={selectedEvent.link} target="_blank" rel="noopener noreferrer" className="text-sm font-black text-indigo-400 hover:text-indigo-300 flex items-center gap-2 truncate">
                 ...
                 <div className="flex items-center gap-5">
                    <div className="p-3 bg-slate-800 rounded-2xl border border-slate-700"><MapPin className="w-6 h-6 text-slate-500" /></div>
                    <div>
                       <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest mb-1">LOCAL</p>
                       <p className="text-base font-black text-white uppercase">AMBIENTE DIGITAL / MEET</p>
                    </div>
                 </div>
                 ...
                 <div className="pt-8 border-t border-slate-800 flex gap-4">
                    <button 
                      ...
                      className="flex-1 py-5 bg-slate-800 hover:bg-indigo-600 text-white rounded-3xl font-black text-[11px] uppercase tracking-widest transition-all shadow-xl shadow-indigo-600/10"
                    >
                      EDITAR
                    </button>
                    <button 
                      onClick={() => { setEvents(prev => prev.filter(e => e.id !== selectedEvent.id)); setSelectedEvent(null); }} 
                      className="px-6 py-5 bg-rose-500/10 hover:bg-rose-600 text-rose-500 hover:text-white rounded-3xl font-black transition-all border border-rose-500/20"
                    >
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 backdrop-blur-2xl animate-in fade-in duration-300">
           ...
           <div className="bg-slate-900 border border-slate-800 w-full max-w-xl rounded-[3.5rem] overflow-hidden relative shadow-2xl animate-in zoom-in-95 duration-300 flex flex-col max-h-[92vh]">
              <div className="p-10 border-b border-slate-800 bg-slate-900/50 flex justify-between items-center">
                 <div className="flex items-center gap-4">
                    <div className="p-3 bg-indigo-600 rounded-2xl shadow-lg">
                      {isEditing ? <Edit2 className="w-6 h-6 text-white" /> : <Plus className="w-6 h-6 text-white" />}
                    </div>
                    <h3 className="text-2xl font-black text-white uppercase tracking-tighter">{isEditing ? 'EDITAR COMPROMISSO' : 'NOVO COMPROMISSO'}</h3>
                 </div>
                 <button onClick={() => setShowAddModal(false)} className="p-3 bg-slate-800 hover:bg-slate-700 rounded-full text-slate-400 transition-all"><X /></button>
              </div>
              <div className="p-10 overflow-y-auto custom-scrollbar space-y-8">
                 <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest">TÍTULO DO EVENTO</label>
                    <input type="text" value={formEvent.title} onChange={e => setFormEvent({...formEvent, title: e.target.value})} className="w-full bg-slate-950 border border-slate-800 rounded-2xl px-6 py-5 outline-none text-white font-black text-xl placeholder:opacity-10 focus:border-indigo-500/40 transition-all" placeholder="Ex: Call Estratégica" />
                 </div>
                 <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-3">
                       <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest">DATA</label>
                       <input type="date" value={formEvent.date} onChange={e => setFormEvent({...formEvent, date: e.target.value})} className="w-full bg-slate-950 border border-slate-800 rounded-2xl px-6 py-4 outline-none text-white font-bold focus:border-indigo-500/40" />
                    </div>
                    <div className="space-y-3">
                       <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest">HORÁRIO</label>
                       <input type="time" value={formEvent.time} onChange={e => setFormEvent({...formEvent, time: e.target.value})} className="w-full bg-slate-950 border border-slate-800 rounded-2xl px-6 py-4 outline-none text-white font-bold focus:border-indigo-500/40" />
                    </div>
                 </div>
                 <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest flex items-center gap-2"><LinkIcon className="w-3.5 h-3.5 text-indigo-400" /> LINK DE ACESSO (GOOGLE MEET / ZOOM)</label>
                    <input type="url" value={formEvent.link} onChange={e => setFormEvent({...formEvent, link: e.target.value})} className="w-full bg-slate-950 border border-slate-800 rounded-2xl px-6 py-4 outline-none text-white font-bold text-sm focus:border-indigo-500/40" placeholder="https://meet.google.com/..." />
                 </div>
                 <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest">CATEGORIA</label>
                    <div className="flex gap-3">
                       {['Work', 'Personal', 'Meeting'].map(type => (
                          <button 
                            key={type} 
                            onClick={() => setFormEvent({...formEvent, type: type as any})} 
                            className={`flex-1 py-4 rounded-2xl border font-black text-[10px] uppercase tracking-widest transition-all ${formEvent.type === type ? 'bg-indigo-600 border-indigo-500 text-white shadow-lg shadow-indigo-600/20' : 'bg-slate-950 border-slate-800 text-slate-600 hover:border-slate-700'}`}
                          >
                 ...
                 <button onClick={handleSaveEvent} className="w-full py-6 bg-indigo-600 hover:bg-indigo-700 text-white rounded-[2.5rem] font-black text-xs uppercase tracking-[0.3em] shadow-2xl shadow-indigo-600/30 active:scale-95 transition-all border border-indigo-500/30">CONSOLIDAR NA AGENDA</button>
              </div>
           </div>
        </div>
      )}
