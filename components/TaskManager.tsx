    ...
    <div className="h-full flex flex-col space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 px-2">
        <div>
          <h2 className="text-4xl font-black text-white tracking-tighter uppercase flex items-center gap-3">
             <CheckSquare className="w-8 h-8 text-indigo-500" /> Fluxo de Trabalho
          </h2>
          <p className="text-slate-500 text-sm font-medium">Sincronizado automaticamente com sua VPS.</p>
        </div>
        <div className="flex gap-4 w-full md:w-auto">
          <div className="relative flex-1 md:flex-none">
            ...
            <input 
              ...
              className="bg-slate-900 border border-slate-800 rounded-2xl py-3 pl-12 pr-6 text-xs w-full md:w-64 text-white outline-none focus:border-indigo-500 transition-all" 
              value={search} 
          ...
          <button 
            ...
            className="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl flex items-center gap-2 border border-indigo-500/30 active:scale-95 transition-all"
          >
      ...
      <div className="flex-1 overflow-x-auto pb-10 custom-scrollbar">
        <div className="flex gap-6 min-w-max h-full items-start">
          {statuses.map(col => (
            <div key={col.id} className="w-[380px] bg-slate-900/20 border border-slate-800 rounded-[3rem] p-6 min-h-[60vh] flex flex-col">
              <div className="flex items-center justify-between mb-8 px-4">
                <div className="flex items-center gap-3">
                  <div className={`w-3 h-3 rounded-full ${col.color} shadow-lg shadow-black/40`}></div>
                  <h3 className="font-black text-xs uppercase tracking-[0.2em] text-slate-400">{col.label}</h3>
                </div>
                <span className="text-[10px] font-black bg-slate-950 px-3 py-1 rounded-full text-slate-600 border border-slate-800">{filteredTasks.filter(t => t.status === col.id).length}</span>
              </div>
                ...
                {filteredTasks.filter(t => t.status === col.id).map(task => {
                  ...
                  return (
                    <div 
                      ...
                      className="bg-slate-900 border border-slate-800 p-6 rounded-[2.5rem] hover:border-indigo-500/50 hover:bg-slate-800/40 transition-all cursor-pointer group shadow-xl relative overflow-hidden"
                    >
                      <div className="flex justify-between items-start mb-4">
                        <span className={`text-[8px] font-black uppercase tracking-[0.2em] px-3 py-1.5 rounded-xl border flex items-center gap-2 ${prio.color} ${prio.bg}/10 border-${prio.value === 'High' ? 'rose' : prio.value === 'Medium' ? 'amber' : 'emerald'}-500/20`}>
                           <Zap className="w-3 h-3" /> {prio.label}
                        </span>
                        <button 
                          onClick={(e) => { e.stopPropagation(); setTasks(prev => prev.filter(t => t.id !== task.id)); }} 
                          className="opacity-0 group-hover:opacity-100 p-2 text-slate-600 hover:text-rose-500 transition-all bg-slate-950 rounded-xl border border-slate-800"
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
                      <h4 className="text-lg font-black text-white mb-2 tracking-tight leading-tight uppercase">{task.title}</h4>
                      <p className="text-xs text-slate-500 line-clamp-2 italic mb-6">"{task.description || 'Nenhuma descrição detalhada...'}"</p>
                      
                      <div className="flex justify-between items-center pt-5 border-t border-slate-800/60">
                        <div className="flex items-center gap-2 text-[9px] font-black text-slate-600 uppercase tracking-widest">
                          <Calendar className="w-3.5 h-3.5" /> 
                          {new Date(task.endDate).toLocaleDateString('pt-BR', {day:'2-digit', month:'short'})}
                        </div>
                        <div className="w-8 h-8 bg-slate-950 rounded-xl border border-slate-800 flex items-center justify-center text-[10px] font-black text-indigo-400 shadow-inner">
