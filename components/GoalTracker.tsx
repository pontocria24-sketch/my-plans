...
const GoalTracker: React.FC<Props> = ({ goals, setGoals }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 lg:space-y-10 pb-20 animate-in fade-in duration-700 px-4 sm:px-6 lg:px-8">
      ...
      {/* Header Estratégico */}
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 lg:gap-8 bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 p-6 lg:p-10 rounded-3xl lg:rounded-[3.5rem] relative overflow-hidden backdrop-blur-md shadow-xl dark:shadow-2xl">
        <div className="relative z-10 max-w-2xl">
          <div className="flex items-center gap-3 mb-3 lg:mb-4">
             <div className="p-1.5 lg:p-2 bg-indigo-500/10 rounded-xl border border-indigo-500/20">
                <Target className="w-5 h-5 lg:w-6 lg:h-6 text-indigo-600 dark:text-indigo-400" />
             </div>
             <span className="text-[8px] lg:text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-[0.3em]">Gestão de Resultados</span>
          </div>
          <h2 className="text-2xl md:text-3xl lg:text-5xl font-black text-slate-900 dark:text-white tracking-tighter mb-3 lg:mb-4 uppercase">Diretrizes & Marcos</h2>
          <p className="text-xs lg:text-sm text-slate-500 dark:text-slate-400 font-medium leading-relaxed">
        ...
        {currentList.map(goal => {
          ...
          return (
            <div 
              ...
              className={`bg-white dark:bg-slate-900/30 border border-slate-200 dark:border-slate-800/80 p-6 lg:p-8 rounded-2xl lg:rounded-[3rem] transition-all flex flex-col group relative overflow-hidden cursor-pointer shadow-lg dark:shadow-none ${isExpired ? 'opacity-70 grayscale-[0.2] hover:grayscale-0' : 'hover:border-indigo-500/40 hover:bg-slate-50 dark:hover:bg-slate-900/50'}`}
            >
              <div className="flex justify-between items-start mb-6 lg:mb-8">
                <div className="space-y-2 lg:space-y-3">
                  <div className="flex items-center gap-2">
                    <span className={`text-[7px] lg:text-[8px] font-black uppercase tracking-widest px-2 lg:px-3 py-0.5 lg:py-1 rounded-lg border ${goal.type === 'Yearly' ? 'bg-amber-500/10 text-amber-600 dark:text-amber-500 border-amber-500/20' : 'bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border-indigo-500/20'}`}>
                      {goal.type === 'Monthly' ? 'Mensal' : goal.type === 'Yearly' ? 'Anual' : 'Personalizado'}
                    </span>
                    <span className="text-[7px] lg:text-[8px] font-black uppercase tracking-widest text-slate-500 bg-slate-100 dark:bg-slate-950 px-2 lg:px-3 py-0.5 lg:py-1 rounded-lg border border-slate-200 dark:border-slate-800">
                      {goal.category}
                    </span>
                  </div>
                  <h4 className="text-xl lg:text-2xl font-black text-slate-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors leading-tight uppercase tracking-tighter">{goal.title}</h4>
                </div>
                {isCompleted ? (
                   ...
                   <div className="p-2 lg:p-3 bg-slate-100 dark:bg-slate-950 rounded-xl lg:rounded-2xl border border-slate-200 dark:border-slate-800">
              ...
              <div className="flex-1 space-y-6 lg:space-y-8">
                <div className="flex justify-between items-end">
                  <div className="space-y-1">
                    <p className="text-[8px] lg:text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Aproveitamento</p>
                    <p className={`text-2xl lg:text-3xl font-black tabular-nums ${isCompleted ? 'text-emerald-500 dark:text-emerald-400' : 'text-slate-900 dark:text-white'}`}>{Math.round(progress)}%</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[8px] lg:text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Período</p>
                    <div className="flex items-center gap-1.5 lg:gap-2 text-slate-600 dark:text-slate-300 font-bold text-[8px] lg:text-[10px] uppercase tracking-tighter bg-slate-100 dark:bg-slate-950 px-2 lg:px-3 py-1 lg:py-1.5 rounded-lg lg:rounded-xl border border-slate-200 dark:border-slate-800">
                ...
                <div className="space-y-2 lg:space-y-3">
                   <div className="w-full bg-slate-100 dark:bg-slate-950 h-2 lg:h-3 rounded-full overflow-hidden border border-slate-200 dark:border-slate-800 shadow-inner">
                     <div 
                   ...
                   <div className="flex justify-between items-center text-[8px] lg:text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] px-1">
                      <span>Atual: {goal.currentValue} {goal.unit}</span>
                      <span>Alvo: {goal.targetValue} {goal.unit}</span>
                   </div>
                </div>

                <div className="bg-slate-50 dark:bg-slate-950/60 p-4 lg:p-5 rounded-2xl lg:rounded-[2rem] border border-slate-200 dark:border-slate-800/80 flex items-center justify-between shadow-inner">
                   <div className="flex items-center gap-2 lg:gap-3">
                      <div className="w-8 h-8 lg:w-10 lg:h-10 bg-white dark:bg-slate-900 rounded-lg lg:rounded-xl flex items-center justify-center border border-slate-200 dark:border-slate-800 shadow-sm">
                         <Hourglass className={`w-3.5 h-3.5 lg:w-4 lg:h-4 ${isExpired ? 'text-rose-500' : 'text-amber-500'}`} />
                      </div>
                      <div>
                         <p className="text-[8px] lg:text-[9px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest">Tempo</p>
                         <p className="text-[10px] lg:text-xs font-bold text-slate-700 dark:text-slate-300">
                   ...
                   <span className="text-xs lg:text-sm font-black text-slate-900 dark:text-white tabular-nums">
                      {goal.currentValue}<span className="text-[8px] lg:text-[10px] text-slate-400 dark:text-slate-500 font-bold ml-1 uppercase">{goal.unit}</span>
                   </span>
                </div>
              </div>
