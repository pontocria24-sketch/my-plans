    ...
    <div className="space-y-6 lg:space-y-10 animate-in fade-in duration-700 pb-20 text-slate-900 dark:text-white">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
        ...
        <div className="lg:col-span-8 bg-white dark:bg-slate-900/20 border border-slate-200 dark:border-slate-800/40 p-6 lg:p-10 rounded-2xl lg:rounded-[3rem] shadow-xl dark:shadow-2xl relative overflow-hidden backdrop-blur-sm">
          ...
          <div className="animate-in slide-in-from-bottom-4 duration-500 min-h-[300px] lg:min-h-[420px]">
               ...
               <div className="space-y-3 lg:space-y-4 max-h-[400px] lg:max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                  {events.filter(e => e.date === todayStr).length > 0 ? events.filter(e => e.date === todayStr).sort((a,b) => a.time.localeCompare(b.time)).map(event => (
                    <div 
                      key={event.id} 
                      onClick={() => setSelectedEvent(event)}
                      className="bg-white dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 p-4 lg:p-6 rounded-2xl lg:rounded-[2.5rem] flex items-center justify-between group hover:border-indigo-500/40 transition-all shadow-lg dark:shadow-xl cursor-pointer"
                    >
                       <div className="flex items-center gap-4 lg:gap-6">
                          ...
                          <div>
                             <h4 className="text-base lg:text-lg font-black text-slate-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-all tracking-tight uppercase">{event.title}</h4>
                       ...
                       <button className="hidden sm:block px-6 lg:px-8 py-2 lg:py-3 bg-slate-100 dark:bg-slate-800 group-hover:bg-indigo-600 text-slate-600 dark:text-white group-hover:text-white rounded-xl lg:rounded-2xl text-[8px] lg:text-[10px] font-black uppercase tracking-widest transition-all shadow-lg border border-slate-200 dark:border-slate-700">
                          Ver Detalhes
                       </button>
                    </div>
                  )) : (
                    <div className="py-16 lg:py-24 text-center border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl lg:rounded-[3rem] bg-slate-50 dark:bg-slate-900/10">
                       <BellRing className="w-12 h-12 lg:w-16 lg:h-16 text-slate-300 dark:text-slate-800 mx-auto mb-4 lg:mb-6 opacity-40" />
                       <h4 className="text-lg lg:text-xl font-black text-slate-400 dark:text-slate-600 uppercase tracking-tighter">Dia Livre</h4>
            ...
            {calendarMode === 'Month' && (
              <div className="bg-slate-50 dark:bg-slate-950/50 rounded-2xl lg:rounded-[3rem] border border-slate-200 dark:border-slate-800 p-8 lg:p-12 text-center py-20 lg:py-28 relative overflow-hidden">
                ...
                <h4 className="text-xl lg:text-2xl font-black text-slate-400 dark:text-slate-400 relative z-10 uppercase tracking-widest">Calendário Mensal</h4>
                <p className="text-slate-500 dark:text-slate-600 text-xs lg:text-sm mt-3 max-w-sm mx-auto relative z-10 leading-relaxed font-medium">
      ...
      {selectedEvent && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 backdrop-blur-md animate-in fade-in duration-300">
           ...
           <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-lg rounded-[3rem] overflow-hidden relative shadow-2xl animate-in zoom-in-95 duration-300">
              ...
              <div className="p-8 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
                 <div className="flex items-center gap-4">
                    ...
                    <h3 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tighter">Detalhes da Agenda</h3>
                 </div>
                 <button onClick={() => setSelectedEvent(null)} className="p-3 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-full text-slate-500 dark:text-slate-400 transition-all border border-slate-200 dark:border-slate-700">
                    <X className="w-5 h-5" />
                 </button>
              </div>
              ...
              <div className="p-10 space-y-8">
                 <div>
                    <h2 className="text-3xl font-black text-slate-900 dark:text-white leading-tight mb-2 uppercase tracking-tighter">{selectedEvent.title}</h2>
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    <div className="bg-slate-50 dark:bg-slate-950 p-5 rounded-3xl border border-slate-200 dark:border-slate-800 flex items-center gap-4">
                       <Clock className="w-6 h-6 text-slate-400 dark:text-slate-500" />
                       <div>
                          <p className="text-[9px] font-black text-slate-500 dark:text-slate-600 uppercase tracking-widest">Horário</p>
                          <p className="text-lg font-black text-slate-900 dark:text-white">{selectedEvent.time}</p>
                       </div>
                    </div>
                    <div className="bg-slate-50 dark:bg-slate-950 p-5 rounded-3xl border border-slate-200 dark:border-slate-800 flex items-center gap-4">
                       <CalendarIcon className="w-6 h-6 text-slate-400 dark:text-slate-500" />
                       <div>
                          <p className="text-[9px] font-black text-slate-500 dark:text-slate-600 uppercase tracking-widest">Data</p>
                          <p className="text-lg font-black text-slate-900 dark:text-white">{new Date(selectedEvent.date + 'T00:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' })}</p>
                       </div>
                    </div>
                 </div>

                 <div className="space-y-4">
                    <div className="flex items-start gap-4 p-5 bg-slate-50 dark:bg-slate-950/40 rounded-3xl border border-slate-200 dark:border-slate-800">
                       ...
                       <div>
                          ...
                          <p className="text-sm font-bold text-slate-700 dark:text-slate-300">Sala de Reuniões Virtual #04 / Dashboard Remoto</p>
                       </div>
                    </div>
                    <div className="flex items-start gap-4 p-5 bg-slate-50 dark:bg-slate-950/40 rounded-3xl border border-slate-200 dark:border-slate-800">
                       ...
                       <div>
                          ...
                          <p className="text-sm font-medium text-slate-600 dark:text-slate-400 leading-relaxed italic">"Compromisso sincronizado com o ecossistema myplans. Foco total nos resultados planejados."</p>
                       </div>
                    </div>
                 </div>
                 ...
                 <button 
                  ...
                  className="w-full bg-slate-900 dark:bg-slate-800 hover:bg-indigo-600 text-white py-5 rounded-3xl font-black text-[11px] uppercase tracking-[0.2em] transition-all shadow-xl shadow-slate-950/20 active:scale-95"
                 >
                   Fechar Visualização
                 </button>
              </div>
