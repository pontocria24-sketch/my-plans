  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700 pb-20 text-white">
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between px-2">
          <div className="flex items-center gap-3">
             ...
             <h3 className="text-xl font-black text-white uppercase tracking-tighter">Foco do Mês Atual</h3>
          </div>
          <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest bg-indigo-500/10 px-4 py-1.5 rounded-full border border-indigo-500/20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {displayGoals.length > 0 ? displayGoals.map(goal => {
            ...
            return (
              <div key={goal.id} className="bg-slate-900/40 border border-slate-800/60 p-6 rounded-[2.5rem] hover:bg-slate-900/60 transition-all group relative overflow-hidden shadow-2xl">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h4 className="font-black text-white text-lg tracking-tight group-hover:text-indigo-400 transition-colors">{goal.title}</h4>
                  ...
                  <div className="text-right">
                    <p className="text-lg font-black text-indigo-400">{Math.round(progress)}%</p>
                  </div>
                </div>
                ...
                <div className="mt-5 flex justify-between items-center">
                  ...
                  <span className="text-[10px] font-bold text-slate-400">{goal.currentValue} / {goal.targetValue} {goal.unit}</span>
                </div>
              </div>
            );
          }) : (
            <div className="col-span-full py-12 text-center bg-slate-900/20 border-2 border-dashed border-slate-800 rounded-[2.5rem]">
              <Target className="w-12 h-12 text-slate-800 mx-auto mb-4" />
              <p className="text-slate-500 font-bold uppercase tracking-widest text-xs">Nenhuma meta ativa para exibição.</p>
            </div>
          )}
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, i) => (
          <div key={i} className="bg-slate-900/20 border border-slate-800/40 p-8 rounded-[2.5rem] hover:border-indigo-500/30 transition-all group shadow-xl">
            <div className={`p-4 rounded-2xl ${stat.bg} w-fit mb-5 group-hover:scale-110 transition-transform`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 bg-slate-900/20 border border-slate-800/40 p-8 sm:p-10 rounded-[3rem] shadow-2xl relative overflow-hidden backdrop-blur-sm">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-12 gap-6 relative z-10">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1 font-medium italic">Seus compromissos organizados com precisão.</p>
            </div>
          ...
          <div className="animate-in slide-in-from-bottom-4 duration-500 min-h-[420px]">
            {calendarMode === 'Week' && (
              <div className="grid grid-cols-7 gap-3">
                {weekDays.map((day, idx) => {
                  ...
                  return (
                    <div key={idx} className={`flex flex-col rounded-[2.5rem] p-3 transition-all border group ${
                      isToday ? 'bg-indigo-600/10 border-indigo-500/50 ring-1 ring-indigo-500/20' : 'bg-slate-900/40 border-slate-800/60'
                    }`}>
                      <div className="text-center mb-6">
                        <p className={`text-[8px] font-black uppercase tracking-[0.2em] ${isToday ? 'text-indigo-400' : 'text-slate-600'}`}>
                          {day.toLocaleDateString('pt-BR', { weekday: 'short' })}
                        </p>
                        <p className={`text-2xl font-black mt-0.5 ${isToday ? 'text-white' : 'text-slate-500'}`}>
                          {day.getDate()}
                        </p>
                      </div>
                      ...
                      <div className="space-y-2 flex-1">
                        {dayEvents.map(event => (
                          <div 
                            ...
                            className={`p-2.5 rounded-2xl border cursor-pointer hover:scale-105 active:scale-95 transition-all shadow-sm ${getEventStyles(event.type)}`}
                          >
                             <div className="flex items-center gap-1.5 mb-1.5 opacity-90">
                                {getEventIcon(event.type)}
                                <span className="text-[8px] font-black uppercase tracking-widest">{event.time}</span>
                             </div>
                             <p className="text-[9px] font-black leading-tight line-clamp-2 uppercase tracking-tighter">
                               {event.title}
                             </p>
                          </div>
                        ))}
               ...
               <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                  {events.filter(e => e.date === todayStr).length > 0 ? events.filter(e => e.date === todayStr).sort((a,b) => a.time.localeCompare(b.time)).map(event => (
                    <div 
                      key={event.id} 
                      onClick={() => setSelectedEvent(event)}
                      className="bg-slate-950/60 border border-slate-800 p-6 rounded-[2.5rem] flex items-center justify-between group hover:border-indigo-500/40 transition-all shadow-xl cursor-pointer"
                    >
                       <div className="flex items-center gap-6">
                          <div className={`w-16 h-16 rounded-2xl flex flex-col items-center justify-center border shadow-inner transition-all group-hover:scale-105 ${getEventStyles(event.type)}`}>
                             {getEventIcon(event.type)}
                             <span className="text-xs font-black mt-1">{event.time}</span>
                          </div>
                          <div>
                             <h4 className="text-lg font-black text-white group-hover:text-indigo-400 transition-all tracking-tight">{event.title}</h4>
                             <div className="flex items-center gap-4 mt-2">
                                <span className={`flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest border px-3 py-1 rounded-full ${getEventStyles(event.type)}`}>
                                   {event.type}
                                </span>
                                <span className="flex items-center gap-1.5 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                                   <MapPin className="w-3 h-3 text-slate-600" /> Digital Meeting
                                </span>
                             </div>
                          </div>
                       </div>
                       <button className="px-8 py-3 bg-slate-800 group-hover:bg-indigo-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shadow-lg">
                          Ver Detalhes
                       </button>
                    </div>
                  )) : (
                    <div className="py-24 text-center border-2 border-dashed border-slate-800 rounded-[3rem] bg-slate-900/10">
                       <BellRing className="w-16 h-16 text-slate-800 mx-auto mb-6 opacity-40" />
                       <h4 className="text-xl font-black text-slate-600">Dia Livre</h4>
                       <p className="text-slate-500 font-bold uppercase text-[10px] tracking-widest mt-2">Foco total nas suas tarefas hoje.</p>
                    </div>
                  )}
               </div>
            )}
            ...
            {calendarMode === 'Month' && (
              <div className="bg-slate-950/50 rounded-[3rem] border border-slate-800 p-12 text-center py-28 relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-indigo-500/5 to-transparent"></div>
                <LayoutGrid className="w-20 h-20 text-indigo-500/20 mx-auto mb-8 relative z-10" />
                <h4 className="text-2xl font-black text-slate-400 relative z-10 uppercase tracking-widest">Calendário Mensal</h4>
                <p className="text-slate-600 text-sm mt-3 max-w-sm mx-auto relative z-10 leading-relaxed font-medium">
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <div className="bg-slate-900 border border-slate-800 p-8 rounded-[3.5rem] shadow-2xl flex-1 relative overflow-hidden group">
             <div className="absolute -top-10 -right-10 p-8 opacity-[0.03] group-hover:opacity-10 transition-opacity duration-1000">
                <CalendarDays className="w-64 h-64 text-indigo-500" />
             </div>
             
             <h3 className="text-xl font-black text-white mb-8 tracking-tight flex items-center gap-3">
                <CalendarDays className="w-6 h-6 text-indigo-400" /> Progresso da Semana
             </h3>

             <div className="space-y-8 relative z-10">
                <div className="text-center py-10 bg-slate-950/50 rounded-[3rem] border border-slate-800 shadow-inner group-hover:border-indigo-500/20 transition-all">
                   <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">Acumulado na Semana</p>
                   <div className="flex items-center justify-center gap-4">
                      <div className={`p-2 rounded-xl bg-indigo-500/10`}>
                        <TrendingUp className="w-6 h-6 text-indigo-400" />
                      </div>
                      <span className={`text-6xl font-black tracking-tighter tabular-nums text-white`}>
                         {(weekStats.totalWeekMs / 3600000).toFixed(1)}<span className="text-3xl ml-1 text-slate-600">h</span>
                      </span>
                   </div>
                   <div className={`mt-5 px-6 py-2 rounded-full w-fit mx-auto text-[10px] font-black uppercase tracking-widest bg-indigo-500/10 text-indigo-400 border border-indigo-500/20`}>
                      Jornada em Evolução
                   </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-end">
                    <span className="text-[10px] text-slate-500 font-black uppercase tracking-widest">Aproveitamento</span>
                    <span className="text-3xl font-black text-indigo-400 tabular-nums">{Math.round(weekStats.performance)}%</span>
                  </div>
                  <div className="h-5 w-full bg-slate-950 rounded-full overflow-hidden border border-slate-800/60 p-1 shadow-inner">
                    <div 
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-950/40 p-5 rounded-[1.5rem] border border-slate-800 text-center group-hover:border-slate-700 transition-all shadow-sm">
                    <p className="text-[9px] font-black text-slate-600 uppercase mb-1 tracking-widest">Meta Semanal</p>
                    <p className="text-xl font-black text-white">{(weekStats.weeklyTargetMs / 3600000).toFixed(0)}h</p>
                  </div>
                  <div className="bg-slate-950/40 p-5 rounded-[1.5rem] border border-slate-800 text-center group-hover:border-slate-700 transition-all shadow-sm">
                    <p className="text-[9px] font-black text-slate-600 uppercase mb-1 tracking-widest">Saldo Pendente</p>
          ...
          <div className="bg-indigo-600 p-8 rounded-[3.5rem] shadow-2xl shadow-indigo-600/30 text-white flex items-center justify-between group cursor-pointer overflow-hidden relative active:scale-95 transition-all">
             ...
             <div className="flex items-center gap-5 relative z-10">
                ...
                <div>
                   <p className="text-[10px] font-black uppercase tracking-[0.3em] mb-1 text-indigo-200">Acesso Premium</p>
                   <h4 className="text-2xl font-black tracking-tighter">Central de Ponto</h4>
                </div>
             </div>
             <CalendarDays className="w-12 h-12 opacity-50 group-hover:scale-110 transition-transform duration-500" />
          </div>
        </div>
      </div>
