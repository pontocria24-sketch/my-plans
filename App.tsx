...
const App: React.FC = () => {
  ...
  useEffect(() => { localStorage.setItem('myplans_tasks_v3', JSON.stringify(tasks)); }, [tasks]);
  useEffect(() => { localStorage.setItem('myplans_ideas_v3', JSON.stringify(ideas)); }, [ideas]);
  useEffect(() => { localStorage.setItem('myplans_goals_v3', JSON.stringify(goals)); }, [goals]);
  useEffect(() => { localStorage.setItem('myplans_events_v3', JSON.stringify(events)); }, [events]);
  useEffect(() => { localStorage.setItem('myplans_scripts_v3', JSON.stringify(scripts)); }, [scripts]);
  useEffect(() => { localStorage.setItem('myplans_worklogs_v3', JSON.stringify(workLogs)); }, [workLogs]);
  useEffect(() => { localStorage.setItem('myplans_config_v3', JSON.stringify(userConfig)); }, [userConfig]);
  useEffect(() => { localStorage.setItem('myplans_auth', isLoggedIn.toString()); }, [isLoggedIn]);
      ...
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 relative overflow-hidden">
        ...
        <div className="w-full max-w-md animate-in fade-in zoom-in-95 duration-500">
          <div className="flex flex-col items-center mb-10">
            <AppLogo isOpen={true} />
            <p className="text-slate-500 mt-4 font-medium text-center">Sua central estratégica de planejamento e produtividade.</p>
          </div>
          ...
          <div className="bg-slate-900/40 border border-slate-800 p-8 rounded-[2.5rem] backdrop-blur-xl shadow-2xl">
            <h2 className="text-2xl font-black text-white uppercase tracking-tighter mb-8 flex items-center gap-3">
              <Lock className="w-5 h-5 text-indigo-400" /> Acesso Restrito
            </h2>
            ...
            <div className="mt-8 pt-8 border-t border-slate-800 flex items-center justify-center gap-2 text-[10px] font-black text-slate-600 uppercase tracking-widest">
              <ShieldCheck className="w-4 h-4 text-indigo-500/40" /> Ambiente Criptografado
            </div>
          </div>
        </div>
      </div>
